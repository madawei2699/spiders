# -*- coding: utf-8 -*-

from scrape.items import INSTAPicItem
import datetime
import os
import re
import scrapy
import time


LOCAL_FILE_SCHEMA = 'file://'
ARCHIVE_PATH = '/archd/archive/instagram_pics/'


def extractLikes(response):
    regexp = '//meta[starts-with(@name, "description")]/@content'
    try:
        content = response.xpath(regexp).extract()[0]
    except (ValueError, IndexError):
        return []

    m = re.search('([\d,mk]+) Likes', content)
    if not m:
        return []

    likes = m.group(1)

    if likes[-1] == 'm':
        return int(float(likes[:-1].strip().replace(',', '')) * 1000000)
    elif likes[-1] == 'k':
        return int(float(likes[:-1].strip().replace(',', '')) * 1000)
    else:
        return int(float(likes.strip().replace(',', '')))


def extractBrotherText(response, text):
    regexp = '//span[contains(text(), "%s")]/span[1]/text()' % text
    try:
        text = response.xpath(regexp).extract()[0].replace(',', '')
    except (ValueError, IndexError):
        return 0

    if text:
        if text[-1] == 'm':
            return int(float(text[:-1].strip().replace(',', '')) * 1000000)
        elif text[-1] == 'k':
            return int(float(text[:-1].strip().replace(',', '')) * 1000)
        else:
            return int(float(text.strip().replace(',', '')))
    else:
        return 0


class INSTAPicsSpider(scrapy.Spider):
    name = 'INSTA_PICS'
    allowed_domains = []
    # To be populated in constructor.
    start_urls = None

    def __init__(self, epoch=None, db_host=None, db_user=None, db_passwd=None):
        assert epoch
        assert db_host
        assert db_user
        assert db_passwd

        self.db_host = db_host
        self.db_user = db_user
        self.db_passwd = db_passwd

        self.timestamp = time.mktime(
            datetime.datetime.strptime(epoch, '%Y%m%d%H%M').timetuple())
        self.log('Crawling page from epoch timestamp: %d' % self.timestamp,
                 level=scrapy.log.INFO)

        # Find all the data files to scrape from.
        self.start_urls = [(LOCAL_FILE_SCHEMA + ARCHIVE_PATH + epoch + '/' + f)
                           for f in os.listdir(ARCHIVE_PATH + epoch)]

    def parse(self, response):
        fn = response.url.split('/')[-1]
        ps = fn.split('-')
        item = INSTAPicItem(
            acct=ps[0],
            uid=ps[-2],
            likes=extractLikes(response),
            views=extractBrotherText(response, ' views'))
        print item
        yield item
